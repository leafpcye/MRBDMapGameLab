...
| PERM-BOOT-01 | Determine whether trusted Sensors + Location requests register the MRBD Universal Menu Permissions item and whether a menu change affects the same low-accuracy call | Version 0.2.4 installed at explicit root `index.html`; request not yet run in this document | Run Bootstrap once; record initial result; middle pinch; record/change menu; Resume; run Verify Location After Menu Change once | Initial and post-menu requests retain separate, same-option evidence; menu result is recorded without assuming API success | Silent stage, repeated Sensors, overwritten initial result, fabricated menu claim, or missing error details | Full JSON plus Phase 1B.4 result template | Desktop harness, MRBD |
